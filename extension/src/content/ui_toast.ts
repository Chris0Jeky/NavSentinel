...
function ensureHost() {
  ...
  style.textContent = `button:focus-visible,.pill:focus-visible{outline:2px solid #f5a623;outline-offset:2px}.head,.pill{display:flex;align-items:center;gap:8px}.wrap,.pill{pointer-events:auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI Variable','Segoe UI',system-ui,sans-serif;box-shadow:0 8px 28px rgba(0,0,0,.4),0 0 0 1px rgba(245,166,35,.15);background:linear-gradient(180deg,#110f13 0%,#08070a 100%);border:1px solid #2a2530;animation:ns-slide-up .2s ease-out}.wrap{width:360px;border-radius:12px;color:#f6efe1;overflow:hidden}@keyframes ns-slide-up{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}.head{padding:10px 12px 0}.head-dot{width:6px;height:6px;border-radius:50%;background:#f5a623;box-shadow:0 0 8px rgba(245,166,35,.5);animation:pulse 1.6s infinite}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}.head-label{font-size:9px;text-transform:uppercase;letter-spacing:.14em;color:#756a5a;font-weight:500}.body{padding:8px 12px 10px;font-size:13px;line-height:1.4;color:#c4b69c}.row{display:flex;gap:8px;padding:10px 12px 12px;border-top:1px solid #1c181f;justify-content:flex-end;flex-wrap:wrap}button{all:unset;cursor:pointer;padding:6px 10px;border-radius:6px;background:rgba(255,255,255,.05);border:1px solid #2a2530;font-size:11px;font-weight:500;color:#c4b69c;transition:background .12s}button:hover{background:rgba(255,255,255,.1)}.danger{background:rgba(208,69,49,.12);border-color:rgba(208,69,49,.3);color:#d04531}.danger:hover{background:rgba(208,69,49,.2)}.action{background:rgba(245,166,35,.1);border-color:rgba(245,166,35,.25);color:#f5a623}.action:hover{background:rgba(245,166,35,.18)}.brief-recovery{width:min(280px,calc(100vw - 24px));display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;border-radius:9px}.brief-recovery .head{padding:0 0 0 10px}.brief-recovery .head-label{display:none}.brief-recovery .body{padding:8px 7px;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.brief-recovery .row{padding:5px 7px 5px 0;border:0;flex-wrap:nowrap}.brief-recovery button{padding:4px 7px;font-size:10px}.recovery-docked{position:fixed;top:12px;right:12px;width:auto;min-width:0;grid-template-columns:auto auto;border-radius:999px;animation:none}.recovery-docked .head{padding-left:8px}.recovery-docked .body{position:absolute;width:1px;height:1px;padding:0;margin:-1px;clip:rect(0 0 0 0);border:0}.recovery-docked .row{padding:4px 5px 4px 0}.pill{border-radius:999px;color:#c4b69c;padding:8px 14px;font-size:13px;cursor:pointer;transition:opacity .4s ease;opacity:1}.pill:hover{opacity:1}.pill.idle{opacity:.45}.pill-count{color:#f5a623;font-weight:600}@media (prefers-reduced-motion:reduce){.wrap,.pill{animation:none;transition:none}.head-dot{animation:none}}`;
...
function renderFullCard(opts: ToastOptions): void {
  ...
  // One lifecycle timer and listener serve both brief and retained recovery.
  // Retained expiry only docks the live control; it never exercises Undo.
  const expire = () => {
    if (!opts.retainedRecovery) {
      remove(!opts.briefRecovery);
      return;
    }
    if (removed) return;
    detachOutsidePointer();
  ...
  if (opts.retainedRecovery || (opts.briefRecovery && opts.timeoutMs !== 0)) {
    outsidePointerDown = (event: PointerEvent) => {
      if (event.isTrusted && !event.composedPath().includes(wrap)) expire();
    };
  ...
  if (t > 0) timeout = window.setTimeout(expire, t);
}
