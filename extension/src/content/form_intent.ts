...
export function submitControlForm(control: HTMLElement): HTMLFormElement | null {
...
export function validateFormReceiver(form: HTMLFormElement, submitter?: HTMLElement | null): void {
  ...
  const isButton = submitter.localName === "button" && buttonType.call(submitter) === "submit";
  const isInput = submitter.localName === "input" && ["submit", "image"].includes(inputType.call(submitter));
  if (!isButton && !isInput) throw new TypeError("The specified element is not a submit button.");
...
export function resolveFormIntent(form: HTMLFormElement, submitter?: HTMLElement | null): FormIntent | null {
  ...
  if (["_self", "_top", "_parent", "_blank"].includes(lower)) target = lower;
  let targetScope: FormIntent["targetScope"] = "other";
  if (!target || target === "_self" || (!["_top", "_parent", "_blank"].includes(target) && target === view.name)) targetScope = "self";
...
export class FormAttemptGate {
  ...
  revoke(): { attemptId?: string; gestureTime?: number } {
    const pending = this.pending;
    this.pending = null;
    return pending ? { ...(pending.attemptId ? { attemptId: pending.attemptId } : {}), gestureTime: pending.gestureTime } : {};
  }
  clear(): string | undefined { return this.revoke().attemptId; }
  consume(form: HTMLFormElement, submitter: HTMLElement | null, intent: FormIntent, now: number): { allowed: boolean; attemptId?: string; gestureTime?: number } {
    const pending = this.pending;
    this.pending = null;
    ...
    return { allowed, ...(pending ? { gestureTime: pending.gestureTime } : {}), ...(pending?.attemptId ? { attemptId: pending.attemptId } : {}) };
  }
}
