...
for(const [mode,file,title]of modes){
 const scripts=[read('shared/core.js'),read('shared/scenarios.js'),`globalThis.NS_INITIAL_MODE=${JSON.stringify(mode)};`,read('web/app.js')].map(x=>x.replace(/<\/script/gi,'<\\/script'));
