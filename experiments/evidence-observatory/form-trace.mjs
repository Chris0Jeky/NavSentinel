...
const faultCodes = ['RUNNER_FAILED','CLOCK_INVALID','RECEIVER_UNHEALTHY','RECEIVER_CALLBACK_LOSS','PAGE_ERROR','CLEANUP_FAILED','PROBE_REJECTED','EVENTS_DROPPED','PRODUCT_READ_FAILED'];
...
export function parseFormTrace(raw, source, makeCase) {
  keys(raw,['schema','mode','scenarioId','variant','pairId','runId','protectedArm','identity','completed','browserVersion','requiredObservationMs','dropped','gaps','events','evidencePolicy']);
  one(raw.schema,[FORM_SCHEMA]);one(raw.mode,['synthetic']);one(raw.scenarioId,['issue688-form-intent']);one(raw.variant,variants);
  ...
  for(const [i,e]of raw.events.entries()){
    keys(e,['id','sequence','elapsedMs','source','kind','data']);if(e.sequence!==i+1||e.id!==`e${i+1}`)fail('EVENT_ORDER_INVALID');
    ...
    switch(e.kind){
      ...
      case 'form.intent':
        ...
        snapshots.push({eventId:e.id,eventIndex:i,elapsedMs:e.elapsedMs,...data});explanation='The authored fixture reported this intent snapshot. Page reports are not proof of native execution or receiver acceptance.';break;
    ...
    c.events.push({id:e.id,sequence:e.sequence,source:e.source,kind,data,elapsedMs:e.elapsedMs,clock:'collector-receive',frame:e.source==='page'?'child':e.kind==='navigation.committed'?e.data.scope:'none',causes:[],explanation});
  }
  ...
  c.formEvidence={pairId:raw.pairId,runId:raw.runId,protectedArm:raw.protectedArm,sourceTree:raw.identity.tree,snapshots,rejectedAttempts,health,dropped:raw.dropped};
  c.warnings.push('FORM_DIAGNOSTIC_NOT_FOUR_ARM_CERTIFICATION','Page snapshots are best-effort reports; receive order is not native execution order or authenticated document identity.','No raw-input verifier or authenticated producer is established by importing these hashes.','A receiver arrival proves the synthetic request boundary, not credential contents.');
  return [c];
}
export function compareFormCases(cases){
  const groups=new Map();for(const c of cases.filter(c=>c.formEvidence)){const id=c.formEvidence.pairId;if(!groups.has(id))groups.set(id,[]);groups.get(id).push(c);}
