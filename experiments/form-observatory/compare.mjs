...
const FORM_TRACE_SCHEMAS=new Set(['navsentinel.observatory.form.v1','navsentinel.observatory.form.v2']);
export function validateFormTraces(rows){
 ...
 for(const row of Array.isArray(rows)?rows:[]){
  ...
  if(!FORM_TRACE_SCHEMAS.has(row.schema))differences.push(`FORM_TRACE_SCHEMA:${key}`);
  if(row.schema==='navsentinel.observatory.form.v2'&&(row.bindingPolicy!=='CDP_DEFAULT_WORLD_DOCUMENT'||row.experiment!=='form-campaign'))differences.push(`FORM_TRACE_CONTRACT:${key}`);
  ...
  if(row.schema==='navsentinel.observatory.form.v2'&&!has('document.started'))differences.push(`FORM_TRACE_DOCUMENT_START:${key}`);
  if(row.schema==='navsentinel.observatory.form.v2'&&!has('document.ended'))differences.push(`FORM_TRACE_DOCUMENT_END:${key}`);
