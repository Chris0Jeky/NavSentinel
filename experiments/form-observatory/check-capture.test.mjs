...
test('all fixed arm captures pass only the capture check, not protection',()=>{const r=checkDocumentCapture(fixtures());assert.equal(r.passed,true);assert.equal(r.records,45);assert.equal(r.evidencePolicy,'CAPTURE_COMPLETENESS_NOT_PROTECTION_EVIDENCE');});
