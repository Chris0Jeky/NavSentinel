...
const arms: readonly Arm[] = [
  ...
  // #864: the same submit inside the click's own task. Child frames get no
  // same-task allowance, and a bare-element click declares nothing anyway.
  ...
  // Accepted #864 residual: the MAIN world arms the same-task allowance only in
  // the top frame, because a page window-capture handler can rewrite a child
  // frame's declared action before a MAIN document-capture listener reads it.
  // A child frame's declared submit in the click's own task therefore stays
  // gated, as it was before #864; the one-task-deferred arm above still passes.
  { id: "benign-declared-form-submit-sync", primitive: "declared-form-submit", delayMs: 0, input: "physical-click", layer: "visible", role: "benign", expectation: "blocked-pre-harm" },
