...
test("guided operator showcase with the real popup surface @demo @demo-operator", async ({}, testInfo) => {
  ...
  try {
    ...
    await expect(optionsPage.locator('#navModeSeg .seg-btn[data-value="strict"]')).toHaveAttribute("aria-checked", "true");
    await expect(optionsPage.locator('#credModeSeg .seg-btn[data-value="strict"]')).toHaveAttribute("aria-checked", "true");
