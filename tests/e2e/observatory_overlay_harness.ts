...
import { chromium, expect, type BrowserContext, type Frame, type Page, type Worker, type TestInfo } from "@playwright/test";
...
export function verifyCampaignInputs(): { source: SourceInputs; artifact: string } {
...
export async function runOverlayArm(arm: Arm, churn: boolean, instrumentation: "full" | "minimal", options: OverlayOptions = {}): Promise<{ run: CapturedRun; browserVersion: string; error: string | null }> {
  ...
  const recorder = createRunRecorder({ runId, arm, contextId, harmTargetId: harmId, benignTargetId: benignId, requiredMs: REQUIRED_MS, instrumentation, maxEvents: options.fault === "page-report-flood" ? 64 : 2000 });
  ...
  try {
    ...
    context = await chromium.launchPersistentContext(profile, { channel: "chromium", headless: true, viewport: options.viewport ?? { width: 1280, height: 900 }, proxy: { server: fence.proxyServer }, args: browserArgs(arm !== "baseline") });
    ...
    const sample = async (): Promise<void> => {
      if (instrumentation === "full" && !skipScene) recorder.record("browser", "scene.sample", { scene: await sampleScene(page, frame, frameContext, harmUrl, benignUrl), frame: "child", context: frameContext(frame) });
    };
    await sample();
    // Exercise the declared reinsertion schedule, not a first-hidden-only oracle.
    if (churn || options.clickDelayMs) await page.waitForTimeout(options.clickDelayMs ?? 1600);
    ...
    await sample();
    if (options.fault && !["page-report-flood", "receiver-observer-error"].includes(options.fault)) await inject();
    ...
    unsubscribe(); removeFaultObserver();
    for (const resource of [sinkClosed ? null : sink, fence, gym]) {
...
export async function attachOverlayTrace(testInfo: TestInfo, start: ReturnType<typeof verifyCampaignInputs>,
  runs: CapturedRun[], browserVersion: string, variantId: string, fileName: string, parameters: unknown = {}) {
  ...
  const trace = { schema: TRACE_V2, mode: "synthetic", campaignId: `${testInfo.testId.replace(/[^a-zA-Z0-9._-]/g, "-")}-${variantId}`, scenarioId: SCENARIO,
    variantId, identity: { repositoryHead: start.source.head, extensionSha256: start.artifact, fixtureSha256: fixtureHash(), browserVersion, profile: "interaction-only", seed: "overlay-timing-v1" },
    provenance: { sourceTree: start.source.tree, inputsBefore: start.source.digest, inputsAfter: after.digest, artifactAfter,
      lockSha256: start.source.lockSha256, settingsSha256: sha(JSON.stringify({ ...SETTINGS, parameters })), rawVerified }, runs };
  const output = testInfo.outputPath(fileName); fs.writeFileSync(output, JSON.stringify(trace, null, 2));
  await testInfo.attach(fileName.replace(/\.json$/, ""), { path: output, contentType: "application/json" });
  expect(rawVerified, "Source or artifact drift invalidated the campaign").toBe(true);
  return trace;
}
export const consequenceVector = (runs: CapturedRun[]) => runs.map(r => ({ arm: r.arm,
  harm: r.events.filter(e => e.kind === "sink.receipt" && e.consequence === "harm").length,
  benign: r.events.filter(e => e.kind === "sink.receipt" && e.consequence === "benign").length,
  controls: r.events.filter(e => e.kind === "control.completed").length }));
