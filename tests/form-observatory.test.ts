...
const report = (recorder: FormObservation, phase: string, primitive: string) => recorder.pageReport({ phase, primitive, intent: sample() });
const completeActionSubstitution = (recorder: FormObservation) => {
  recorder.browser("143.0.7499.4");
  recorder.health("start", { ok: true, sequence: 1 });
  report(recorder, "prepared", "native");
  report(recorder, "input", "native");
  report(recorder, "operation", "submit");
  recorder.health("end", { ok: true, sequence: 2 });
};
...
it("certifies a completed run only after the fixed arm sequence, browser and health evidence", () => { const r = make(); completeActionSubstitution(r); const t = r.finish(true); expect(t.completed).toBe(true); expect(t.gaps).toEqual([]); });
it("does not let earlier prepared and input reports hide a lost operation report", () => { const r = make(); r.browser("143.0.7499.4"); r.health("start", { ok: true, sequence: 1 }); report(r, "prepared", "native"); report(r, "input", "native"); r.health("end", { ok: true, sequence: 2 }); const t = r.finish(true); expect(t.completed).toBe(false); expect(t.gaps).toContain("OBSERVATION_INCOMPLETE"); });
it("known delivery gaps and dropped reports contradict completion", () => { const r = make(); completeActionSubstitution(r); r.fail("RECEIVER_CALLBACK_LOSS"); const t = r.finish(true); expect(t.completed).toBe(false); expect(t.gaps).toContain("RECEIVER_CALLBACK_LOSS"); });
it("page cannot impersonate a receiver or manufacture a gap", () => { const r = make(); r.pageReport({ source: "sink", phase: "input", intent: sample(), primitive: "native" }); const t = r.finish(true); expect(t.dropped).toBe(0); expect(t.gaps).not.toContain("PROBE_REJECTED"); expect(t.events.some(e => e.source === "sink")).toBe(false); });
it("unknown sensitive fields are refused without manufacturing a gap", () => { const r = make(); r.pageReport({ phase: "input", intent: sample(), primitive: "native" }); r.pageReport({ phase: "input", intent: { ...sample(), password: "SECRET" }, primitive: "native" }); const t = r.finish(true); expect(t.dropped).toBe(0); expect(t.gaps).not.toContain("PROBE_REJECTED"); expect(t.events.filter(e => e.kind === "form.intent")).toHaveLength(1); expect(JSON.stringify(t)).not.toContain("SECRET"); });
...
it("serialized probe leaves missing delivery for runner-side completeness checks", () => { const script = formProbeScript(harm, benign); const run = new Function("f", "a", "globalThis", script + ";__nsFormReport('operation',f,a,'requestSubmit');"); expect(() => run(form, button, {})).not.toThrow(); expect(script).not.toContain("__nsFormObservationFault"); });
it("serialized probe handles a rejected callback without a forgeable fault binding", async () => { const script = formProbeScript(harm, benign); const run = new Function("f", "a", "globalThis", script + ";__nsFormReport('operation',f,a,'requestSubmit');"); run(form, button, { __nsFormObservation: () => Promise.reject(new Error("receiver")) }); await Promise.resolve(); expect(script).not.toContain("__nsFormObservationFault"); });
it("ignores malformed page reports without manufacturing an observer gap", () => {
 const r = make(); r.pageReport({phase:{toString:()=>"input"},primitive:"native",intent:sample()});
 const t = r.finish(false); expect(t.dropped).toBe(0); expect(t.gaps).not.toContain("PROBE_REJECTED");
});
