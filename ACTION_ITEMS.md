...
**Last updated:** 2026-06-13 (session 2) · by Claude — merged **#247** (North-Star docs; 10 bot review findings fixed) + **#248** (failure-ledger autolog hook fix). **#249 OPEN** = P5-C1 / **#238** (replay-grade `PromptOutcomeEntry`): green CI + 2 adversarial review rounds, **2444 unit tests**, all perf budgets pass — **awaiting your Gate-3 manual Chrome test + merge.** `main` @ `02d8ded`. AI-3 ✅ · AI-4 ✅ web-ext · **AI-5 open (supply/sanction reference brand *logos*)**.
...
- **(2026-06-13 session 2 — CURRENT):** `main` == **`02d8ded`**. This session: merged **#247** (North-Star roadmap/research docs — fixed all 10 gemini+codex review findings first) and **#248** (failure-ledger hygiene: the `PostToolUseFailure` hook now writes auto-captures to a **gitignored `failure_autolog.jsonl`**, the curated `failure_ledger.jsonl` was scrubbed from 78 lines [71 were `unclassified` machine noise] to **7 real entries**, and `agent:hooks:smoke` was made branch-aware so it passes on feature branches). **#249 OPEN** = P5-C1 / **#238** (replay-grade `PromptOutcomeEntry`: adds `cds`/`nrsFactors`/`navAnomalyScore`/`adaptiveAdj`/`thresholdUsed`/`elementContext`, fixes nav-drops-`reasons` + cred-lacks-`destDomain`): **green CI (Build/Unit + E2E), 2 adversarial review rounds (self-run workflow + Codex) with all findings fixed, 2444 unit tests, all 12 perf budgets pass** — **awaiting Gate-3 manual Chrome test + merge.** **Gotcha learned:** `npm run check:perf-budget` is a CI-only gate (not in `test`/`lint`/`typecheck`/`build`); it caught a `capture_isolated` budget tip-over — run it locally for any extension change.
