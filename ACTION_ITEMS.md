...
- **(2026-06-14 Codex pickup - P5-B1 latest):** PR **#253** now includes commit `fff9cda` fixing the latest Codex review findings plus the Linux perf-budget follow-up: same-tab silent navigation events are appended by the service worker only after a matching top-frame commit, JS-driven allowed same-tab navigations get the same commit-confirmed path, true new-window `window.open` remains immediate while `_self`/`_top`/`_parent` avoids duplicate logging, `ns-nav-allowed` is no longer debug-gated, host-bearing non-web schemes are rejected, and the `capture_isolated` cap is documented at 63KB after CI measured 62.6KB. Local verification: focused silent/SW tests 67 pass, `typecheck`, `build`, `lint`, perf budget 12/12 (`capture_isolated` 62.6KB / 63KB), full unit 2467 pass with known happy-dom/network stderr, rollback E2E 3 pass. GitHub Build/Unit is green and E2E/re-review are running on #253. Human-owned OPEN items remain **AI-5** and **AI-6**.

- **(2026-06-14 Codex pickup - P5-A4 update):** PR **#256** is open for P5-A4 / #235 (`feat/p5a4-container-intent`, latest commit `a31287f`). Scope: container-aware `intent_mismatch_under_interactive` suppression for structural navigation containers only when the top container actually contains the underlying action; sibling/full-page/delegated overlays without containment remain blockable. Two independent local adversarial reviews both found the high-severity delegated overlay evasion in the first implementation; fixed before opening PR. Gemini later asked for `hasActionIntent` cursor fallback robustness; fixed by treating cursor values ending with `pointer` as action intent and sharing that semantics with `cursor_pointer_no_affordance`. Local verification after latest fix: focused scoring/property tests 96 pass, `typecheck`, `lint`, `build`, perf budget 12/12 (`capture_isolated` 61.0KB / 62KB; total dist 448.8KB / 500KB), full unit 2433 pass with known happy-dom/network stderr. CI/re-review now running on #256. Human-owned OPEN items remain **AI-5** and **AI-6**.
...
- **(2026-06-14 Codex pickup - CURRENT):** `main` == **`a68958c`** (`#251` merged the Vite-8 Gym serve fix). Reused canonical orchestrator `docs/agentic/ORCHESTRATOR.md`. Open PRs are **#249** (P5-C1 / #238; green CI + 2 review rounds, waiting on AI-6 manual Chrome Gate-3), **#253** (P5-B1 / #236 silent-decision events; latest review fixes and CI perf-budget follow-up pushed through `fff9cda`; Build/Unit green, E2E/re-review running), **#254** (draft docs/CI PR refreshing `AGENTS.md` against `CLAUDE.md`; Gemini formatting finding fixed in `e0fa07c`; redundant Xvfb apt step removed after hosted runner Microsoft feed 403 broke E2E; `agent:skills:validate` pass; Build/Unit green, E2E running after latest status push), **#255** (P5-A2 / #233 Smart Mode blank-prompt suppression; keyboard, Live OAuth suppressor, and OAuth monitor parity fixes through `4a77b39`; CI green/re-review running), and **#256** (P5-A4 / #235 container intent heuristic; Gemini cursor-fallback finding fixed through `a31287f`; CI/re-review running). Human-owned OPEN items remain **AI-5** and **AI-6**.
