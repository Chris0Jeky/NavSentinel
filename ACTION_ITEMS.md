...
- **(2026-06-14 Codex pickup - P5-B1 latest):** PR **#253** now includes merge commit `ad91b16` on top of `main` after **#255** merged. Latest functional fixes remain `9a93684` plus perf-budget follow-up `fff9cda`: same-tab silent navigation events are appended by the service worker only after a matching top-frame commit, JS-driven allowed same-tab navigations get the same commit-confirmed path, true new-window `window.open` remains immediate while `_self`/`_top`/`_parent` avoids duplicate logging, `ns-nav-allowed` is no longer debug-gated, host-bearing non-web schemes are rejected, and the `capture_isolated` cap is documented at 63KB. Local verification after merging `main`: focused silent/SW/Smart/OAuth tests 159 pass, `typecheck`, `build`, `lint`, perf budget 12/12 (`capture_isolated` 62.6KB / 63KB), full unit 2484 pass with known happy-dom/network stderr, rollback E2E 3 pass. CI/re-review are running on #253. Human-owned OPEN items remain **AI-5** and **AI-6**.
...
- **(2026-06-14 Codex pickup - CURRENT):** `main` == **`69400fc`** after **#255** merged. Reused canonical orchestrator `docs/agentic/ORCHESTRATOR.md`. Open PRs are **#249** (P5-C1 / #238; green CI + 2 review rounds, waiting on AI-6 manual Chrome Gate-3), **#253** (P5-B1 / #236 silent-decision events; latest branch commit `ad91b16` merges `main`; CI/re-review running), **#254** (draft docs/CI PR refreshing `AGENTS.md` against `CLAUDE.md`; latest branch has merged `main`; CI/re-review running), and **#256** (P5-A4 / #235 container intent heuristic; latest branch commit `fcb6f4d` merges `main`; CI/re-review running). Human-owned OPEN items remain **AI-5** and **AI-6**.
