...
- **Clear behavioural data (limited scope).** The Options reset clears the event log, prompt outcomes, adaptive per-domain scores and domain behaviour profiles (#535). Navigation-category profiles and smart-default cooldown pairs remain outside its scope and have no current clear control (RI-06/#474 remains open).
...
- **Overlay cleanup activity in Options Analytics.** An "Overlays auto-dismissed" metric counts the cleanups retained in the local event log, split into page-settle, injected-overlay and blocked-click paths. It reads only existing local events (#935).
- **Per-section settings reset in storage.** A storage operation resets one settings section to its defaults; there is no UI for it yet (#939).
...
- **JavaScript-behaviour instrumentation is off in every build.** The `fetch`, `XMLHttpRequest`, `sendBeacon` and password-value monitors (#101, #104, #105, #107, #108, #402, #506) are now a declared release-profile capability set to false in all profiles, including the release build (#532). Its dormant form-destination and credential signals was hardened for a future opt-in build (#930).
...
- **Tests:** broader unit and property-based coverage across scoring, storage, domain handling, analyzers and UI models (#72, #84, #87, #103, #121, #122, #123, #124, #125, #126, #136, #137, #138, #139, #140, #141, #142, #143, #144, #147, #148, #149, #150, #152, #155, #159, #160, #161, #162, #163, #164, #278, #519, #550, #551, #749, #751, #752, #754, #915, #923).
- **Browser test lanes:** the Phase-2 end-to-end lane was repaired and is selected by default again, mutation-observer and bridge-ordering tests were made deterministic, headed lanes run serially, and branded-Chrome acceptance lanes were added (#112, #503, #504, #524, #543, #544, #574, #576, #602, #603, #607, #611, #623, #625, #682, #697, #730, #733, #734, #735, #743, #845, #846, #861, #871, #881, #893, #917, #919).
- **Release-integrity regression tests:** interaction-only builds reject reputation artifacts, packages without GPL terms are refused, and the unpacked research profile rejects missing, corrupt, or unexpectedly exposed reputation assets (#907, #908, #909).
- **Browser evidence stability:** acceptance fixtures now bring trusted controls into view, wait for the actual popup and gauge state, locate bridge logic across built chunks, and enter child frames before trusted clicks (#903, #904, #905, #906).
- **Local test ranges (not shipped):** the Gym, Proving Ground and adversarial programme now use loopback-only destinations and typed harm receipts, the phishing corpus scores "protected" separately from "fired", and a maintainer-run branded-Chrome runner was added (#70, #251, #435, #597, #598, #605, #610, #613, #614, #615, #616, #617, #618, #619, #620, #621, #624, #626, #627, #628, #629, #630, #631, #632, #633, #634, #635, #654, #663, #666, #669, #681, #683, #687, #714, #912).
...
- **CI:** workflows run on Node 24 actions, use read-only tokens by default, pin actions to commits, run on stacked pull requests, bound job durations, and fail on auxiliary-lane flakes (#512, #538, #670, #674, #675, #676, #677, #678, #679, #732, #747). A scheduled, advisory branded-Chrome lane and a label-triggered branded browser check for pull requests were added (#911, #913, #937).
...
- **An approved blocked form submit is refused if its destination changed.** When a blocked `form.submit()` or `requestSubmit()` is approved, including the automatic approval for an allowlisted destination, the form must still point at the approved URL, so changing `action` or `formaction` after the block no longer works (#897). A change made inside the page's own `submit` or `formdata` handler can still redirect it (issue #898, open).
...
- **Local data minimisation:** the event-log `url` field is reduced to origin and path (#468), likely-secret path segments are redacted (#517), and prompt outcomes keep hostnames only (#518). These rules apply to new records, imports and exports, and existing data is migrated. Mutation-alert `extra.details` can still retain full URLs in this release (#902; fix proposed in #910).
