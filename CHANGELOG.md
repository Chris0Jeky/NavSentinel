...
- **An approved blocked form submit goes only where it was approved.** When a blocked `form.submit()` or `requestSubmit()` is approved, including the automatic approval for an allowlisted destination, the form must still point at the approved URL. A page can no longer change `action` or `formaction` after the block (#897).
