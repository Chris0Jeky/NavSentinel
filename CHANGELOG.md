...
- **Forms a page submits from its own click handler are no longer blocked.** A declared form submitted synchronously or in a microtask by the page's handler for a trusted click (for example a "Log in with Google" button that triggers `submit`, validation libraries, or `<a onclick="form.submit()">`) now gets the same allowance as one submitted a task later. Only trusted clicks the isolated world allows arm it, within the existing 1.5-second, two-submit budget (#895). A form posted into the page's own named iframe is still blocked (issue #865, open).
