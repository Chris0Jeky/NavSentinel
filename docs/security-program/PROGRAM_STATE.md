...
- 69 current-work mappings: 12 Gym levels, 23 RW journeys, 12 evasion fixtures, 5 ClickFix fixtures, 4 DoubleClickjacking fixtures, 11 evidence lanes, and 2 browser-suite mappings.
...
All 168 canonical scenarios remain `UNMODELLED`. Existing artefacts are reconciled separately: 60 mappings are `MODELLED`, 2 are `UNMODELLED`, and 7 bounded mappings are `REGRESSION_PROVEN`. No scenario is claimed as browser-proven, efficacy-measured, release-eligible, universally secure, or anonymous.
...
The #523 vertical adds a bounded deterministic queue campaign plus matched vulnerable and protected bundled-Chromium bridge-pressure sequences. The browser baseline loads an ephemeral copy of the exact built artifact with only the coalescing function disabled and proves that the real bridge loses the later critical receipt; the normal artifact retains the receipt and reports no true overflow. Benign and mixed controls keep legitimate clipboard use available and the verified ClickFix signal observable. A second controlled bridge reconnect proves that the next lifecycle window contains no earlier clipboard receipt and completes without overflow. These are product-event and queue observations with no independent harm oracle, so the C-04 lane remains `MODELLED`, not `REGRESSION_PROVEN`. The mutant is test-only and outside repository build output; it is not a production bypass. The queue lives in the document's MAIN world before isolated/SW delivery, so service-worker restart is not its state boundary; #175/#186 still own general bridge identity, liveness, and recovery. AI-37's explicit queue-policy decision and owner-Chrome run remain open.
