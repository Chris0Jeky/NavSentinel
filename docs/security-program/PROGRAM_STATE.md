...
The #566 repair lane uses trusted bundled-Chromium input on loopback-only `127.0.0.1` and `localhost` fixtures. Navigation Off reaches the opener-navigation sink. Current-context early-event and effective-target repairs preserve the requested child without granting modified anchors service-worker or MAIN opener authority. Base-target, named-context, same-site, non-HTTP, child-frame, and closed-shadow handlers and native behavior remain reachable where applicable. Page-origin popups from those non-isolated handlers are blocked as a known MEDIUM false intervention. Behavior head `03f04f4` passed the 30-case headed matrix, five focused target-classifier units, both unchanged profile budgets, and hosted run `33318797927`. Later qualification campaigns supersede that receipt: run `33320724757` was `HARNESS_INVALID` because direct evaluation crossed document replacement and generic page attribution could select extension onboarding. The corrected web-first and exact-loopback-child methodology then exposed one valid final-destination survivor in each of two separate 50-profile campaigns. A Chrome-owned source-baseline acknowledgement candidate still produced one survivor in 50 and exceeded the unchanged research `capture_isolated` budget, so it was not pushed. Rare possible rollback redelivery remains a separate lifecycle residual. The mapped lane is therefore `FIXTURE_PROVEN`, PR #572 is parked on SP-F-013 before AI-31, and pointerup/mouseup remain unseen holdouts. The broader C-02 and canonical WIN-004 evidence states do not advance.
