...
The #523 vertical adds a bounded deterministic queue campaign plus matched vulnerable and protected bundled-Chromium bridge-pressure sequences. The browser baseline loads an ephemeral copy of the exact built artifact with only the coalescing function disabled, proves that the real bridge loses the later critical receipt, and reaches a typed loopback sink as `HARM_REACHED`; the normal artifact retains the receipt, reports no true overflow, and keeps that sink empty. Benign and mixed controls keep legitimate clipboard use available and the verified ClickFix signal observable. A second controlled bridge reconnect proves that the next lifecycle window contains no earlier clipboard receipt and completes without overflow. This gives the C-04 lane `REGRESSION_PROVEN` evidence only. The mutant is test-only and outside repository build output; it is not a production bypass. The queue lives in the document's MAIN world before isolated/SW delivery, so service-worker restart is not its state boundary; #175/#186 still own general bridge identity, liveness, and recovery. AI-37's explicit queue-policy decision and owner-Chrome run remain open.
