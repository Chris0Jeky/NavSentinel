...
All 168 canonical scenarios remain `UNMODELLED`. Existing artefacts are reconciled separately: 60 mappings are `MODELLED`, 2 are `UNMODELLED`, one #566 modifier-click opener-authority mapping is `FIXTURE_PROVEN`, and six bounded mappings are `REGRESSION_PROVEN`. No scenario is claimed as browser-proven, efficacy-measured, release-eligible, universally secure, or anonymous.
...
The #566 modifier-click opener-authority lane remains `FIXTURE_PROVEN`, not browser-proven. PR #572 remains parked before AI-31 on SP-F-013; green CI or automated Chromium does not substitute for Chris's manual Gate-3, and no owner Chrome result is inferred here. The current-main integration keeps the branch-held procedure in `ACTION_ITEMS.md`.
