...
Record `TEST_INVALID` when the sink is not armed, the extension build/profile is wrong, readiness is unknown, a public request occurs, a non-synthetic value reaches the sink, timing attribution is ambiguous, or the control fixture cannot complete. Preserve the receipt and fix the harness before drawing a security conclusion.
