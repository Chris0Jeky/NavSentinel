...
The `main_guard` script runs in the page's own execution context — the only place where you can intercept `window.open()` and `form.submit()`. It patches those browser primitives at the prototype level, wrapping them in gesture-gated checks. Every such navigation attempt needs a valid gesture token from the Watcher, delivered through a private MessagePort channel. `location.assign()` / `location.replace()` are **not** in that set and cannot be: Chromium exposes them as non-writable, non-configurable own methods of each `Location`, so no page-context script can wrap them (#458). Script redirects through `location` are never blocked before they commit; the service worker's rollback layer recovers *some* of them after the fact — a top-frame redirect that crosses to a different registrable domain, or a same-domain one that follows a recent user navigation, when no allowance covers it. A same-domain script redirect with no recent user navigation is deliberately left alone. Full conditions: `docs/Architecture_and_Data_Flow.md`.
