...
**Repo state (live pointer 2026-07-03):** `main` @ **`f2c3652`** (verify with `git rev-parse origin/main` + `gh pr list`). **Direction adopted: ship/measure, not hardening** — Priority Ladder, milestone-gated discovery, and icebox in [`docs/agentic/DECISIONS.md`](agentic/DECISIONS.md). Baseline: **2856 unit tests** (94 files), perf 12/12, CI green. **3 open PRs, all human-gated at the WIP cap:** #399 (AI-14) / #356 (AI-13) / #273 (AI-8). Last session merged #429 (claims-honesty #423) + #430 (release guard + `release.mjs` syntax fix). Live human tasks + snapshot in [`ACTION_ITEMS.md`](../ACTION_ITEMS.md); cycle log in [`docs/agentic/ORCHESTRATOR.md`](agentic/ORCHESTRATOR.md); next-loop entry in [`docs/agentic/HANDOFF.md`](agentic/HANDOFF.md). The checkpoints below are historical.
