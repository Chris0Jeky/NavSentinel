...
**Repo state (live pointer 2026-06-27):** `main` @ **`b6629f1`** (verify with `git rev-parse origin/main`); **2 open PRs — #356 (AI-13) + #273 (AI-8), both browser-Gate-3-held.** For the full session-by-session log see `ACTION_ITEMS.md` (current-state snapshot) and [`docs/agentic/ORCHESTRATOR.md`](agentic/ORCHESTRATOR.md) (cycle log; Cycles 35-36 = the 2026-06-26/27 **8-PR** session #365/#368/#369/#370/#371/#372/#375/#376). The deferred questions Q1-Q6 are now **answered** (see ACTION_ITEMS): **Q2 = split+bump the `capture_isolated` budget** — the cap is now **70KB** (bump done; the split is tracked in **#374**, reclaim after). The session-3 checkpoint below is **historical**.
