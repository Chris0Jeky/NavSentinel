...
**Codex pickup status (verified 2026-06-14):** `main` @ **`69400fc`** after merging **#255** (P5-A2 Smart Mode blank-prompt suppression). Open PRs: **#249** (P5-C1 / #238, human Gate-3), **#253** (P5-B1 / #236, latest branch commit `b52f7f6` after a CI-only capture budget fix, CI/re-review running), **#254** (draft Codex contract/status refresh plus CI Xvfb apt hardening, latest branch merged `main`, CI/re-review running), **#256** (P5-A4 / #235 container intent heuristic, latest branch commit `fcb6f4d` merges `main`, CI/re-review running).
...
| P5-B1 / #236 silent-decision events | `feat/p5b1-silent-decision-events` | `main` | root checkout | #253 | done | done | pending after `b52f7f6` | Latest review comments fixed; branch merged `main` after #255. Linux CI then measured `capture_isolated` at 64.4KB / 63KB; `b52f7f6` raises only that cap to 65KB with rationale. Local verification after merge: focused tests 159 pass, typecheck, build, lint, perf-budget 12/12, full unit 2484 pass, rollback E2E 3 pass. Budget follow-up: build plus perf-budget 12/12 (`capture_isolated` 64.4KB / 65KB; total dist 455.4KB / 500KB) |
| Codex contract/status refresh | `docs/codex-contract-refresh` | `origin/main` | `../NavSentinel-codex-contract` | #254 | Gemini done | Codex done | Build/Unit green; E2E pending | Draft docs/CI PR; aligns `AGENTS.md` with `CLAUDE.md`, carries autonomous-loop status sync, and removes redundant Xvfb apt install after hosted-runner Microsoft feed 403 broke E2E |
