...
**Codex pickup status (verified 2026-06-14):** `main` @ **`a68958c`** after #251 (`gym:serve` Vite-8 fix). Open PRs: **#249** (P5-C1 / #238, human Gate-3), **#253** (P5-B1 / #236, review fixes pushed through `42fff89`, CI/re-review running), **#254** (draft Codex contract/status refresh, CI green before latest status sync), **#255** (P5-A2 / #233 Smart Mode blank-prompt suppression, keyboard and Live OAuth fixes pushed through `6e3421e`, CI/re-review running).
...
| P5-B1 / #236 silent-decision events | `feat/p5b1-silent-decision-events` | `main` | root checkout | #253 | done | done | pending after `42fff89` | Review comments fixed: null-safe logs, SW-backed append durability, delegated retries, corrupted-row pruning, serialized event writes; local verification green |
...
| P5-A2 / #233 Smart Mode blank-prompt suppression | `feat/p5a2-signal-smart-gating` | `origin/main` | `../NavSentinel-p5a2-smart-gating` | #255 | local security review done | local runtime review done | pending after `6e3421e` | Gemini keyboard-activation and Codex Live OAuth feedback fixed; local verification: typecheck, lint, build, focused unit 16 pass, targeted E2E 3 pass, perf budget 12/12, full unit 2442 pass |
