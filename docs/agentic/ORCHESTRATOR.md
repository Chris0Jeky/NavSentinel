...
**Codex pickup status (verified 2026-06-14):** `main` @ **`a68958c`** after #251 (`gym:serve` Vite-8 fix). Open PRs: **#249** (P5-C1 / #238, human Gate-3), **#253** (P5-B1 / #236, review fixes and perf-budget follow-up through `fff9cda`, Build/Unit green, E2E/re-review running), **#254** (draft Codex contract/status refresh plus CI Xvfb apt hardening, Build/Unit green, E2E running after latest status push), **#255** (P5-A2 / #233 Smart Mode blank-prompt suppression, fixes pushed through `4a77b39`, CI green/re-review running), **#256** (P5-A4 / #235 container intent heuristic, Gemini cursor-fallback finding fixed through `a31287f`, CI/re-review running).
...
| P5-B1 / #236 silent-decision events | `feat/p5b1-silent-decision-events` | `main` | root checkout | #253 | done | done | Build/Unit green after `fff9cda`; E2E pending | Latest review comments fixed: commit-confirmed same-tab silent events, JS-driven allowed nav coverage, web-scheme filter, debug-free allowed bridge; Linux perf budget follow-up raised `capture_isolated` cap to 63KB with local verification green |
...
| P5-A4 / #235 container intent heuristic | `feat/p5a4-container-intent` | `origin/main` | `../NavSentinel-p5a4-container-intent` | #256 | local scoring review done | local coverage review done | pending after `a31287f` | Both local reviews found delegated/sibling navigation-container overlay evasion; fixed before PR by requiring contained underlying action. Gemini cursor fallback finding fixed by treating cursor values ending with `pointer` as action intent. |
