...
- **Non-browser** (agent may merge on the standard T2 gate) = shared pure-logic with unit tests, scripts, build config, tests, docs, CI.
  *(Amended 2026-07-31 after agent-harness #91/#101: the original wording restated a fixed merge gate — "aging + 2 adversarial rounds + green CI + bot comments addressed" — that the global laws now own. The T2 gate is green proving checks at the reviewed head, the current risk-calibrated review and comment-triage pass, and a three-minute final-head merge-eligibility floor. That floor is not a polling cadence and does not create a review round per push. The browser/non-browser classification below is the part of this decision that remains repo truth.)*
