...
> **Authority:** these encode maintainer-confirmed direction. Where they refine the autonomous-loop prompt or older rules, they win; a returning agent should treat them as current posture.
...
3. **Measure & serve day-one users:** the measurement-enablement set (#416/#417/#418/#423/#426) and the North-Star spine slices that serve first users (#232, #237, explanations, #239/#240).
...
**Dependency (stated plainly):** the stricter tier only works if Gate-3 has a cadence — either a committed periodic maintainer sitting *or* the headed lane (D-2026-07-03-E). Until then the pragmatic posture holds and the North-Star spine (much of which is browser-surface) will throttle through this gate; that is the known cost.
...
- **Q1–Q6 (answered 2026-06-26):** Q1 OAuth FP cluster = implement + HOLD for `measure:fp`; **Q2** capture_isolated = split (#374) **and** bump 66→70KB; Q3 = prefer unit-testable extraction over CI-only e2e (CI-verified e2e acceptable when unavoidable); Q4 merge cadence confirmed; **Q5** Gate-3 = manual Chrome (**now revised by D-2026-07-03-E**); Q6 = extract-to-testable + e2e for `main_guard`.
