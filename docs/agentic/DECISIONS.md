...
- **Non-browser** (agent may merge on the standard T2 gate) = shared pure-logic with unit tests, scripts, build config, tests, docs, CI.
  *(Amended 2026-07-27, agent-harness #91/#101: the original wording restated a merge gate — "aging + 2 adversarial rounds + green CI + bot comments addressed" — that the global laws now own. The T2 gate is green proving checks at the reviewed head plus one comment-triage pass; there is no PR-aging requirement. The browser/non-browser classification below is the part of this decision that remains repo truth.)*
