...
- `main` == **`69400fc`** after #255 (P5-A2 Smart Mode blank-prompt suppression) merged.
- **#249** (P5-C1 / #238) is green, clean, and still blocked only on AI-6 manual Chrome Gate-3 + merge.
- **#253** (P5-B1 / #236) has latest branch commit `b52f7f6` after merging `main` in `ad91b16` and fixing a CI-only capture budget failure. Functional fixes remain `9a93684`: commit-confirmed same-tab silent nav logging, JS-driven allowed nav coverage, non-web scheme filtering, and debug-free allowed bridge. GitHub Linux CI measured the combined #233 + #236 `capture_isolated` chunk at 64.4KB after the merge; `b52f7f6` raises only that cap to 65KB with inline rationale while leaving total dist capped at 500KB. Local verification after merging `main`: focused silent/SW/Smart/OAuth tests 159 pass, typecheck, build, lint, perf-budget 12/12, full unit 2484 pass, rollback E2E 3 pass. Budget follow-up verification: build plus perf-budget 12/12 (`capture_isolated` 64.4KB / 65KB, total dist 455.4KB / 500KB). CI/re-review are running.
...
- **#256** (P5-A4 / #235) has latest branch commit `fcb6f4d`, merging `main` after #255. Scope: container-aware `intent_mismatch_under_interactive` suppression for structural navigation containers only when the top container contains the underlying action; sibling/full-page/delegated overlays without containment remain blockable. Gemini cursor fallback feedback is fixed in `a31287f`: `hasActionIntent` now treats cursor values ending with `pointer` as action intent, and that semantics is shared with `cursor_pointer_no_affordance`. Local verification after merging `main`: focused scoring/dom-builder/Smart/OAuth tests 212 pass, typecheck, lint, build, perf budget 12/12 (`capture_isolated` 61.0KB / 63KB), full unit 2450 pass with known happy-dom/network stderr. CI/re-review are running.
