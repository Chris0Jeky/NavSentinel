...
- **#256** (P5-A4 / #235) has latest branch commit `b69bf69`, refreshed from current `main` after #253. Scope: container-aware `intent_mismatch_under_interactive` suppression for structural navigation containers only when the top container contains the underlying action; sibling/full-page/delegated overlays without containment remain blockable. Local verification after the refresh: focused scoring/dom-builder/Smart/OAuth/silent tests 287 pass, typecheck, lint, build, perf budget 12/12 (`capture_isolated` 65.0KB / 66KB; total dist 456.6KB / 500KB), full unit 2499 pass with known happy-dom/network stderr. GitHub CI is running on `b69bf69`.
