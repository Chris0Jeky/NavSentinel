...
- **#253** (P5-B1 / #236) has latest branch commit `ad91b16`, merging `main` after #255. Functional fixes remain `9a93684` plus perf-budget follow-up `fff9cda`: commit-confirmed same-tab silent nav logging, JS-driven allowed nav coverage, non-web scheme filtering, debug-free allowed bridge, and 63KB `capture_isolated` cap. Local verification after merging `main`: focused silent/SW/Smart/OAuth tests 159 pass, typecheck, build, lint, perf-budget 12/12, full unit 2484 pass, rollback E2E 3 pass. CI/re-review are running.
