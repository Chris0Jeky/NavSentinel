...
- **#253** (P5-B1 / #236) has latest review fixes and CI follow-up pushed through `fff9cda`: same-tab silent nav events now append from the SW only after a matching top-frame commit, JS-driven allowed same-tab navigations use that durable path, true new-window `window.open` remains immediate while same-tab targets avoid duplicates, `ns-nav-allowed` is no longer debug-gated, host-bearing non-web schemes are rejected, and the Linux `capture_isolated` perf budget is documented at 63KB after CI measured 62.6KB. Local verification: focused silent/SW tests 67 pass, typecheck pass, build pass, lint pass, perf-budget 12/12 pass, full unit 2467 pass, rollback E2E 3 pass. GitHub Build/Unit is green; E2E/re-review are running.
- **#254** (Codex contract/status refresh) is a draft docs/CI PR. `AGENTS.md` was aligned with `CLAUDE.md`; Gemini formatting feedback was fixed in `e0fa07c`; status sync is carried on the docs branch. CI E2E failed at `6f97e3a` because hosted Ubuntu Microsoft apt feeds returned 403 during a redundant `apt-get install xvfb` step; fixed by removing that redundant step from CI and stress workflows. GitHub Build/Unit is green and E2E is running after the latest status push.
- **#255** (P5-A2 / #233) is open from `feat/p5a2-signal-smart-gating` at `4a77b39`. Scope: Smart Mode blank-anchor prompt suppression for narrow benign contexts with trusted pointer/click or keyboard gating, NRS block/factor safeguards, curated IdP/payment matching including Microsoft Live OAuth authorize endpoints, OAuth monitor tracking parity for Microsoft Live endpoints, and Gym/E2E regression. Local verification after the latest review fix: focused OAuth/Smart/SW tests 197 pass, typecheck, lint, build, targeted E2E 3 pass, perf budget 12/12 (`capture_isolated` 62.6KB / 63KB), full unit 2443 pass with known happy-dom/network stderr. Two independent local adversarial reviews completed and all findings were addressed; Gemini keyboard and Codex Live OAuth/OAuth-tracker feedback were fixed; CI is green and GitHub review is running.
- **#256** (P5-A4 / #235) is open from `feat/p5a4-container-intent` at `a31287f`. Scope: container-aware `intent_mismatch_under_interactive` suppression for structural navigation containers only when the top container contains the underlying action; sibling/full-page/delegated overlays without containment remain blockable. Two independent local adversarial reviews both found the high-severity delegated overlay evasion in the first implementation; fixed before PR. Gemini cursor fallback feedback is fixed: `hasActionIntent` now treats cursor values ending with `pointer` as action intent, and that semantics is shared with `cursor_pointer_no_affordance`. Local verification after latest fix: focused scoring/property tests 96 pass, typecheck, lint, build, perf budget 12/12 (`capture_isolated` 61.0KB / 62KB), full unit 2433 pass with known happy-dom/network stderr. CI/re-review are running.
