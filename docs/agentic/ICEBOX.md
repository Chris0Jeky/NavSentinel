...
- **#418** (re-bodied 2026-07-03) — the Safe-Browsing / competitor comparison arm for `benchmark.mjs`. The honest re-scope (option b: name + roadmap now say "gym regression, competitor arm unbuilt") shipped; building the actual comparison arm needs a headed Chromium + a bare-profile Safe-Browsing run (network) → gated with the measurement-reset session **#416** (which owns the benchmark-baseline re-run).
