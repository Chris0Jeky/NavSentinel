...
| Opt-in overlay cleanup (#555) | `storage.ts` (`nav.autoDismissOverlays`), Options + popup UI, `overlay_cleanup.ts` | `mutation_monitor.ts` classification and `capture_isolated.ts` action wiring | `tests/overlay-cleanup.test.ts`, storage/toggle/toast/popup-a11y tests, suite-ui + mutation-01 Phase-2 + evasion-02/evasion-11 regression E2E; local high-severity classification, reversible suppression, no click replay; Gate-3 for shipped page/UI behavior. |
