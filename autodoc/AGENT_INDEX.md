...
0. **Resuming the autonomous work loop?** Read `ACTION_ITEMS.md` (human-owned tasks + current-state snapshot), `docs/agentic/HANDOFF.md` (latest session handoff), and `docs/agentic/ORCHESTRATOR.md` (living backlog/cycle log) first. The current `main` SHA is intentionally live-state only; verify it with `git rev-parse origin/main`. As of the 2026-06-14 durable checkpoint, #254, #256, #257, #258, and later docs cleanup are merged; the active PR/gate set is intentionally live-state only — verify open PRs and gates with `gh pr list` and `docs/agentic/HANDOFF.md`. Do not start duplicate work for #233/#234/#235/#236/#238; #233 is closed; live open North-Star issues are #232, #237–#246, and #252. Next candidates are intentionally live-state only — see `docs/agentic/ORCHESTRATOR.md` and `docs/Project_Roadmap.md`.
...
| Gym and E2E harness | `gym/index.html`, `tests/e2e/extension_test_utils.ts` | 122 gym HTML fixtures, 14 E2E specs under `tests/e2e/` | Playwright spec, `npm run gym:serve`. |
