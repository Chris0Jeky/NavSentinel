...
| Popup/options UI | `popup.ts`, `popup_model.ts`, `options.ts` | popup/options CSS/HTML, `design_tokens.css`, `icons.ts`, `event_tone.ts`, `explanations.ts`, `smart_defaults.ts` | popup-model/popup-a11y/toggle-a11y/icons unit tests, suite-ui E2E. UI uses segmented controls (`#navModeSeg`/`#credModeSeg` with `.seg-btn[data-value]` + `aria-pressed`), toggle buttons (`role="switch"` + `aria-checked`), and sidebar nav (`data-section`). Pending PRs #132-#135 add radiogroup pattern, `aria-labelledby`/`aria-describedby`, popup ARIA landmarks, and decorative SVG `aria-hidden`. |
...
| Gym and E2E harness | `gym/index.html`, `tests/e2e/extension_test_utils.ts` | 119 gym HTML fixtures, 11 E2E specs under `tests/e2e/` | Playwright spec, `npm run gym:serve`. |
...
- NavSentinel v0.4.0, Phases 0-3 complete + Phase 4 partial (4/8), 1154 tests (45 unit test files + 11 E2E spec files), 52 source files (48 TS + 4 CSS), 119 gym fixtures.
...
- 22 open PRs (#114-#135) awaiting human review. PRs cover: hygiene/bug fixes, toolchain migration, ESLint, test coverage, type safety, accessibility (ARIA radiogroup, toggle switches, popup landmarks, icon SVG aria-hidden).
- All icon SVGs from `icon()` and `logoSentinel()` now include `aria-hidden="true"` (pending PR #135 merge).
